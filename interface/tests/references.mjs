...
const daily=page.split('aria-label="Navegación diaria"')[1].split('</nav>')[0];
assert.deepEqual([...daily.matchAll(/data-view="([^"]+)"/g)].map(m=>m[1]),['cockpit','projects','library','inbox','notebook']);
const secondary=page.split('id="secondaryNavigation"')[1].split('</details>')[0];
assert.deepEqual([...secondary.matchAll(/data-view="([^"]+)"/g)].map(m=>m[1]),['overview','skills','dashboard']);
